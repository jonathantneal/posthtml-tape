...
const opts = Object.assign(
	...
	// package.json[posthtmlConfig] options
	requireOrThrow(path.join(cwd, 'package.json')).posthtmlConfig,
	// argument options
	...process.argv.filter(
		...
		Object.keys(tests[section]).map(
			(name) => {
				...
				return Promise.all([
					...
					([sourceHTML, expectHTML]) => plugin.process(sourceHTML, test.options).then(
						(result) => writeFile(resultPath, result.html).then(
							() => {
								if (result.html !== expectHTML) {
